...
AVFilterContext *ffw_filtersink_new(AVFilterGraph *filter_graph)
{
    ...
    int ret = avfilter_graph_create_filter(&buffersink_ctx, buffersink, "out",
...
{
    return av_buffersrc_add_frame(context, frame);
}

int ffw_filtergraph_flush(AVFilterContext *context)
{
    return av_buffersrc_add_frame(context, NULL);
}
